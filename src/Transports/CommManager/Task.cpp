    ...
    struct Task: public DUNE::Tasks::Task
    {
      ...
      onMain(void)
      {
        while (!stopping())
        {
          ...
          {
            if (m_vmedium == NULL || m_vmedium->medium != IMC::VehicleMedium::VM_UNDERWATER)
            {
              if (m_args.iridium_plain_texts)
              {
                IMC::ReportControl reportRequest;
                reportRequest.op = IMC::ReportControl::OP_REQUEST_REPORT;
                reportRequest.comm_interface = IMC::ReportControl::CI_SATELLITE;
              ...
              if (msg != NULL)
              {
                ...
                request.setDestination(m_ctx.resolver.id());
