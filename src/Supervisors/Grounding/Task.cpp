    ...
    struct Task: public DUNE::Tasks::Task
    {
      ...
      checkTransectSafety(double lat_start, double lon_start, double lat_end, double lon_end)
      {
        ...
        for(int j=0; j<m_features.size(); j++)
        {
          ...
          {
            features = true;
            //std::string directory = m_args.debug_path + "threats_transect_" + std::to_string(Angles::degrees(lat_end)) + ".csv";
            //m_enc->writeCSVfile(static_threats, directory);
      ...
      checkDepthContours(double usv_lat, double usv_lon)
      {
        ...
        {
          //std::string directory = m_args.debug_path + "depth_contours_" + std::to_string(Angles::degrees(usv_lat)) + ".csv";
          //m_enc->writeCSVfile(depth_contours, directory);
          lats="lats=";
          lons="lons=";
      ...
      onMain(void)
      {
        ...
        //m_enc->writeCSVfile(m_enc->getSquare(DUNE::Math::Angles::radians(63.4531), DUNE::Math::Angles::radians(10.4299), 500.0, 0), m_args.debug_path + "out.csv");
        ...
        while(!stopping())
        {
          ...
          if(m_timer.overflow())
          {
            //! Check vehicle depth surroundings.
            /*ENCManager::DepthSoundingVector depth_surroundings = m_enc->getWithinRadius(m_current_lat, m_current_lon, m_args.range, 0.0, "depthmapRad");
            for(ENCManager::DepthSoundingVector::iterator itr = depth_surroundings.begin(); itr != depth_surroundings.end(); ++itr)
            {
              inf("DEPTH SURROUNDINGS %f %f %f", itr->Lat, itr->Lon, itr->Depth);
            }*/
            ...
            //m_enc->writeCSVfile(depth_surroundings, m_args.debug_path + "surroundings" + std::to_string(m_current_lat) + ".csv");

            /*
            //! Check for static objects in vehicles surroundings.
            PointsOfInterest::LocationVector threat_surroundings = m_poi->getPOIWithinRadius(m_current_lat, m_current_lon, 1000, m_features[0]);
            if(threat_surroundings.empty())
              debug("NO THREATS IN THE SURROUNDINGS!");
            else
            {
              for(LocationData::LocationVector::const_iterator itr = threat_surroundings.begin(); itr != threat_surroundings.end(); ++itr)
              {
                inf("THREAT SURROUNDINGS %f %f", Angles::degrees(itr->Lat), Angles::degrees(itr->Lon));
              }
            }*/
