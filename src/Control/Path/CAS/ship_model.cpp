...
: n_samp_(T/dt)
{
	...
	A_ = 2.5; // [m]  in reality the length is 14,5 m.
	B_ = 2.5; // [m]
	C_ = 0.5; // [m]
	D_ = 0.5; // [m]
	l = (A_ + B_);
...
void shipModel::linearPrediction(const Eigen::Matrix<double,6,1>& state, double u_d, double psi_d){
	...
	*/
	...
	//std::cout << "AutoNaut w/ heading offset: " << (180.0f/M_PI)*psi_d << std::endl;
	for (int i = 0; i < n_samp_-1; i++){
		...
		if (slow_turn){ // Moment of inertia functionality when turning. [Not working properly]
			...
			if (psi(i) < psi_d ){
				...
				//std::cout << "psi = psi_d: " << (180.0f/M_PI)*psi(i) << " = "  << (180.0f/M_PI)*psi_d  << " state(2) " << (180.0f/M_PI)*state(2) << std::endl;
				add = 0;	// Keep psi
			}
			deltaPsi = psi_d - psi(i);
			if (fabs(deltaPsi) > 1.5*(M_PI/180.0)){
				psi(i+1) = normalize_angle(psi(i) + 1.5*(M_PI/180.0)*add);
				//std::cout << i << ": deltaPsi: "  << (180.0f/M_PI)*deltaPsi << " fabs(deltaPsi) " << fabs(deltaPsi) << std::endl;
			}else{
				psi(i+1) = normalize_angle(psi_d);
				//std::cout << "Ref reached - psi(i+1): " << (180.0f/M_PI)*psi(i+1) << " deltaPsi: "  << (180.0f/M_PI)*deltaPsi << std::endl;
			}
		}
		
		else{			// Straight Linear prediction
			psi(i+1) = normalize_angle(psi_d);
		}
		...
		std::cout << i << ".(x,y,psi,u,v): " << x(i) << " " << y(i) << " " << (180.0f/M_PI)*psi(i) <<  " " << u(i) << " " << v(i)  << " psi(i+1) " << (180.0f/M_PI)*psi(i+1) << " psi(i) " << (180.0f/M_PI)*psi(i)  << " deltaPsi: " << (180.0f/M_PI)*deltaPsi << " add: " << add << std::endl;
		*/
	}
	//std::cout << " " << std::endl;
