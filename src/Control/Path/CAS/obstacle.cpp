...
{
	//std::cout << std::endl << "Linear Predicted trajectories w/heading: " << (180.0f/M_PI)*psi_  << std::endl; 
	//std::cout << 0 << ".Obstacle(x,y,u,v): " << x_(0) << " " << y_(0) << " " << u_(0) << " " << v_(0) << std::endl;
	for (int i = 1; i < n_samp_;  i++)
	{
		...
		u_(i) = (u_(i-1));
		v_(i) = (v_(i-1));
				//std::cout << i << ".Obstacle(x,y): " << x_(i) << " " << y_(i) << std::endl;
		//std::cout << i << ".Obstacle(x,y,u,v): " << x_(i) << " " << y_(i) << " " << u_(i) << " " << v_(i) << std::endl;
	}
	//std::cout << " " << std::endl;
