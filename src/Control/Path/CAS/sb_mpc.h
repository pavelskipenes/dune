...
public:
	...
	double getKChi();
	/**
	 * @brief Returns the cost of start with heading offset to starboard. KChiSB > KChiP Ensures a change to SB rather Port
	 */
	double getKChiSB();
	/**
	 * @brief Returns the cost of start with heading offset to port.
		 */
	double getKChiP();
	/**
	...
	double getKdChiP();
	/**
	 * @brief Enable/Disable printing of the Cost.
	 */
	double getPrintCost();
	/**
	 * @brief Returns the possible offsets to the nominal heading [deg].
	...
	void setPrintCost(double print_Cost);
