  ...
  namespace Path
  {
    namespace CAS
    {
      ...
      struct Task: public DUNE::Control::PathController
      {
        ...
        //! Static obstacles matrix from ENC.
        Math::Matrix m_static_obstacles;
        //! Depth soundings matrix from ENC.
        Math::Matrix m_depth_sound;
        ...
        m_reqid(0)
        {
          ...
          .description("Entity label of 'EstimatedFreq' message");
        ...
        onResourceAcquisition(void)
        {
          ...
          m_static_obst_state.resizeAndFill(m_offsets.size(),3,10000.0); //! Large values: initial cost is high.
        }
        ...
        consume(const IMC::AisInfo* msg)
        {
          ...
          /*if(m_enable_cas)
          {
            for(unsigned int i = 0; i < m_dyn_obst_vec.size(); i++)
            {
              debug("Obstacle with MMSI %s, callsign %s, name %s is CLOSER than %0.1f m - LAT %.3f, LON %.3f, COURSE %.3f, NAV_STATUS %d, SOG %.3f, TYPE/CARGO %d, LENGTH: %.3f, WIDTH:%.3f DRAUGHT:%.3f - OBST VECT SIZE %lu", m_dyn_obst_vec[i].mmsi.c_str(), m_dyn_obst_vec[i].callsign.c_str(), m_dyn_obst_vec[i].name.c_str(), m_args.out_of_range, Angles::degrees(m_dyn_obst_vec[i].lat), Angles::degrees(m_dyn_obst_vec[i].lon), Angles::degrees(m_dyn_obst_vec[i].course), m_dyn_obst_vec[i].nav_status, m_dyn_obst_vec[i].speed, m_dyn_obst_vec[i].type_and_cargo, m_dyn_obst_vec[i].a+m_dyn_obst_vec[i].b, m_dyn_obst_vec[i].c+m_dyn_obst_vec[i].d, m_dyn_obst_vec[i].draught, m_dyn_obst_vec.size());
            }
          }*/
        }
        ...
        groundingCost()
        {
          ...
          {
            double bath_cost = m_args.K_ENV[0]*std::pow(bathymetry,2);
            double heave_cost = m_args.K_ENV[1]*std::pow(m_heave,2);
            double wave_freq_cost = m_args.K_ENV[2]*m_wave_freq;
            double wind_cost = m_args.K_ENV[3]*m_wind_speed*std::pow(std::fmax(0, std::cos(psi_path + Angles::radians(m_offsets[i]) - Angles::radians(m_wind_dir))),2);
            double current_cost = m_args.K_ENV[4]*m_current_vel*std::pow(std::fmax(0, std::cos(psi_path + Angles::radians(m_offsets[i]) - Angles::radians(m_current_dir))),2);
            m_static_obst_state(i,2) = m_args.K_GROUND + bath_cost + heave_cost + wave_freq_cost + wind_cost + current_cost;
          }
          //ENCManager::DepthSoundingVector dep_vec = m_enc->getSquare(m_lat_asv, m_lon_asv, m_args.drval2, 5000.0);
          //m_enc->writeCSVfile(dep_vec, m_args.debug_path + "useful_depare.csv");
        }
        ...
        step(const IMC::EstimatedState& state, const TrackingState& ts)
        {
          ...
          //! Compute dynamic obst. matrix for sb_mpc if CAS is enabled and there are dynamic obstacles in range.
          if(C_CAS)
          {
            // Create and fill matrix of dynamic obstacles.
            m_dyn_obst_state.resize(m_dyn_obst_vec.size(), 10);

            //! Update m_asv_states to fit sb_mpc inputs.
          ...
          //! Compute static obst. matrix for sb_mpc if ANTI-GROUNDING is enabled and there are static obstacles in range.
          if(C_AG)
          {
            m_static_obst_state.resizeAndFill(m_offsets.size(),3,10000.0); //! Large values: initial cost is high.
            buildENCforCAS();
            ...
            for(unsigned int i=0; i<m_static_obst_state.rows(); i++)
              debug("m_static_obst_state rows to sb_mpc: %.0f %.3f %.0f",Angles::degrees(m_static_obst_state(i,0)),m_static_obst_state(i,1),m_static_obst_state(i,2));
          }
          ...
          if((C_CAS || C_AG) && (m_timestamp_new - m_timestamp_prev) > 20.0)
          {
            ...
            sb_mpc.getBestControlOffset(u_os, psi_os, m_asv_state[3], m_des_heading.value, m_asv_state, m_waypoints, m_dyn_obst_state, m_static_obst_state, cost);
            ...
            {
              debug("Course offset is %.0f, new desired course %.3f", m_des_heading.off, Angles::degrees(m_des_heading.value));
            ...
            debug("OFFSET: %.0f - NEW COURSE NORMALIZED %.3f",Angles::degrees(psi_os),Angles::degrees(m_des_heading.value));
