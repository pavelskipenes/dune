...
namespace DUNE
{
  namespace Navigation
  {
	...
	void
	simulationBasedMpc::create(double T, double DT, double T_stat, double P, double P_G, double Q, double D_CLOSE, double D_SAFE, double D_SAFE_LAND, double K_COLL, double PHI_AH, double PHI_OT, double PHI_HO, 
	...
	void simulationBasedMpc::getBestControlOffset(double &u_os_best, double &psi_os_best, double u_d, double psi_d_, const std::vector<double>& asv_state, const Eigen::Matrix<double,-1,2>& waypoints_, const Math::Matrix& obst_states, Math::Matrix static_obst_states_, double &cost_mra){
	...
	double simulationBasedMpc::costFunction(double P_ca, double Chi_ca, int k, bool SB_0, bool CRG_0, bool OTG_0, bool OT_0, bool HOT_0, double DIST_0, double u_d, int l, int &ik_return_to_path, Math::Matrix obst_state, Math::Matrix static_obst_state, int chi_ca_index){
		...
		for(int i = 0; i < n_samp-1; i+=pred_step){ // using 5s or 10s fixed intervals for DT_=0.5
			...
			if(static_obst_state(0,0)!=10000.0){
				...
				dist_to_land = static_obst_state(chi_ca_index,1); //dist_to_land = (static_obst_state(chi_ca_index+1,1) != 0.0) ? fmax(1.0, static_obst_state(chi_ca_index+1,1) - asv_pred_dist) : 0.0;
